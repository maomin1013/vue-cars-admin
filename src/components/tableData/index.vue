<template>
  <div>
    ...
    <el-table v-loading="loading_table"
      ...
      <template v-for="item in this.table_config.thead">
        ...
        <!-- switch -->
        <el-table-column v-else-if="item.type === 'switch'"
          :key="item.prop"
          :prop="item.prop"
          :label="item.label"
          :width="item.width">
          <template slot-scope="scope">
            <el-switch @change="item.handler && item.handler($event, scope.row)"
